...
\]
where $p_{i}$ is a binary classifier for label $i$, given the feature vector $\textbf{f}$. The output of a binary classifier $\ell_{i}$ is either 0 or 1. There are $L$ binary classifiers for $L$ labels. $\zeta_{i}$ is the $i^{th}$ output of the classification function $C$ and the classifier output $\ell_{i}$ is 1 if $\zeta_{i}$ is greater than certain threshold $\epsilon$ 
\[
...
\[
\textbf{pred}_{(\epsilon)} = \{ p_{i}(\zeta_{i}) | p_{i}(\zeta_{i}) = 1 \} \qquad i \in \{1,2,..,L\}
\]
...
\]
Without the non-linear operation $\Phi$, the operators $\textbf{W}_{L1}$ and $\textbf{W}$ can multiply out to generalize a single layer perceptron. The motivation for using a two layer perceptron is because single layer perceptron can approximate only linear class boundries. But \textit{universal approximation theorem} [] states that a single hidden layer with some non-linear activation function can approximate any function. The choice of $\Phi$ will be explained in section \ref{training}. The second operator $\textbf{W}$ performs the projection from hidden vector $\textbf{h}$  
\[
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State $\textbf{pred} = \{ p_{i}(\zeta_{i}) | p_{i}(\zeta_{i}) = 1 \} \qquad i \in \{1,2,..,L\}$
  \end{algorithmic}
\end{algorithm}
\FloatBarrier
