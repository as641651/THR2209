...
Now, MFCCs from every STFT window is passed to the two layer LSTM which was used previously. The RNN was first trained with MTT dataset before our target dataset. The parameter setting are same as those used while training \textbf{CNN+RNN}. The resulting WAUC was \textbf{0.74}   
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State $\textbf{Y} = Drop_{(0.3)}(Seq2Seq\_LSTM(\textbf{R}))$ \Comment{$\textbf{Y} \in \mathbb{R}^{1024 \times P}$}
